...
begin
  ...
  if total_tasks=0 then new_score:=1.0; else new_score:=greatest(0.0, least(1.0, (completed_tasks-returned_tasks)::numeric/total_tasks)); end if;
...
begin
  ...
  if action='approve' then
    update tasks set status='done', approved_at=now(), approved_by=auth.uid(), approval_comment=comment where id=task_uuid;
    ...
    update tasks set status='awaiting_photos', returned_for_revision_at=now(), revision_comment=comment where id=task_uuid;
    perform update_reliability_score(task_record.assigned_to);
