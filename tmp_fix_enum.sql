...
begin
  -- считаем по updated_at/approved_at за 30 дней, статусы из существующего enum
  select count(*) into total_tasks
  from tasks 
  where assigned_to = user_uuid
    and status in ('completed','awaiting_approval','returned')
    and coalesce(updated_at, created_at, now()) >= now() - interval '30 days';
  ...
  where assigned_to = user_uuid
    ...
    and coalesce(approved_at, updated_at, created_at, now()) >= now() - interval '30 days';

  if total_tasks = 0 then new_score := 1.0; 
  else new_score := greatest(0.0, least(1.0, (completed_tasks - returned_tasks)::numeric / total_tasks)); end if;
